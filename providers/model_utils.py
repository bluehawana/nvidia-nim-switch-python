"""Model name normalization utilities for NVIDIA NIM.
...
def normalize_model_name(model: str, default_model: Optional[str] = None) -> str:
    ...
    It strips provider prefixes and maps Claude model names to the configured NVIDIA NIM model.
    ...
    # Map Claude models to default NVIDIA NIM model
    if is_claude_model(clean):
        if default_model is None:
            # First check for explicit MODEL environment variable (highest priority for backward compatibility)
            env_model = os.getenv("MODEL")
            if env_model:
                default_model = env_model
            else:
                # Check if we have a dynamically selected NVIDIA NIM model
                try:
                    from config.model_presets import model_presets_manager
                    default_model = model_presets_manager.get_current_model_id()
                except ImportError:
                    # Fallback to hardcoded NVIDIA NIM default
